...
if topology == "old":
    ...
    attack_dict = {15: (2.20, 3.50, 'CT', ' C_W957_W950_1_W957_A'),
                   ...
                   43: (6.2, 7.1, 'Vphase', ' V_BUS13KV1_CN'),
    ...
    if topology == "3B":
        unmonitored_sensors.update({111,112,113})
        del order_attack[-3]
        adj_list =  {69: [26, 15],
                     70: [27, 16],
                     71: [28, 17],
...
def save_results(threshold_corr, threshold_var, rsum_list_corr, rsum_list_var, all_attack_indices,
                 all_attack_indice, total_normal, persistency=5, window=320, w=1,
                 save_path=f"models/results_{topology}.json"):
    violations = np.logical_or(
        (rsum_list_corr > threshold_corr.reshape(-1, 1)), (rsum_list_var > threshold_var.reshape(-1, 1))
    )
    ...
    fpr_l_sensor = []
    for att_time in sorted(list(all_attack_indices)):
        fpr_l_sensor.append(FPR_Local(violations, [att_time], attacks_shifted_index, window, w, persistency))
    ...
    d = {"DD": ADD, "LD": LD, "FPR_L_Mean": fpr_local, "FPR_L_indiv_attack": np.array(fpr_l_sensor), "FLR": flr}
